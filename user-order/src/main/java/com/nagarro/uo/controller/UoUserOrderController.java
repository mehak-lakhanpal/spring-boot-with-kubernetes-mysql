...
public class UoUserOrderController {
	...
	public ResponseEntity<UoUserOrderRes> getUser(@PathVariable Long userId){
		...
		ResponseEntity<UoUserRes> userDetails = template.getForEntity(userUrl+"/user/{userId}", 
				UoUserRes.class, uriVariables);
		ResponseEntity<UoUserOrderRes> orders = template.getForEntity(orderUrl+"/orders/{userId}", 
